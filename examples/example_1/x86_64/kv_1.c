	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 kv /home/mike/dev/neurotune/examples/example_1/x86_64/kv_1.mod\n");
...
static int  rates (  _lv )  
	double _lv ;
 {
   a = - ( 0.616 + 0.014 * _lv ) / ( exp ( ( 44.0 + _lv ) / - 2.3 ) - 1.0 ) ;
   b = 0.0043 / ( exp ( ( 44.0 + _lv ) / 34.0 ) ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 73 in file kv_1.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
